resource "aws_instance" "bastionhost" {
  ...
  volume_tags = "${merge(var.resource_tags,
            map(
              "belongs_to", "${var.resource_prefix}_${terraform.workspace}_${var.resource_name}${count.index + 1}${var.resource_suffix}"
              )
              )}"

  tags = "${merge(var.resource_tags,
            map(
              "Name", "${var.resource_prefix}_${terraform.workspace}_${var.resource_name}${count.index + 1}${var.resource_suffix}"
              )
              )}"
}

resource "aws_placement_group" "bastionhostpgroup" {
  name     = "${var.resource_prefix}bastionhost-pgroup"
  strategy = "spread"
}
